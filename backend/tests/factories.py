"""Factories para o domínio SME (Design Doc v2)."""
...
class UserFactory(DjangoModelFactory):
    ...
    email = factory.LazyAttribute(lambda o: f'user_{o.cpf}@example.com')
...
# Alias legado
AdminUserFactory = SMEAdminFactory
...
class TeacherAllocationFactory(DjangoModelFactory):
    ...
    subject = factory.SubFactory(
        SubjectFactory,
        education_department=factory.SelfAttribute(
            '..school_class.school.education_department'
        ),
    )
    is_regent = False
...
class GradeFactory(DjangoModelFactory):
    class Meta:
        model = 'class_diary.Grade'

    enrollment = factory.SubFactory(EnrollmentFactory)
    subject = factory.SubFactory(
        SubjectFactory,
        education_department=factory.SelfAttribute(
            '..enrollment.student.education_department'
        ),
    )
...
ClassFactory = SchoolClassFactory
ClassDetailFactory = SchoolClassFactory
TeacherFactory = TeacherProfileFactory
GuardianFactory = StudentGuardianUserFactory
